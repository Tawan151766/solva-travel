...
export async function DELETE(request) {
  try {
    console.log('🗑️ Delete API - Request received');
    
    // Check authentication - try NextAuth first, then fallback to legacy auth
    const session = await getServerSession(authOptions);
    console.log('Delete API - NextAuth Session:', session);
    ...
    if (session?.user) {
      user = session.user;
      console.log('Delete API - Using NextAuth user:', user.email);
    } else {
      authHeader = request.headers.get('authorization');
      const bearerToken = authHeader?.startsWith('Bearer ') ? authHeader.substring(7) : null;
      ...
      if (bearerToken) {
        try {
          ...
          console.log('Delete API - Using legacy auth user:', user.email);
        } catch (error) {
          console.log('Delete API - Invalid legacy token:', error.message);
        }
      }
    }
    
    if (!user) {
      console.log('Delete API - No valid authentication found');
      return NextResponse.json(
    ...
    const cloudinaryService = new CloudinaryService();
    const deleteResult = await cloudinaryService.deleteFile(fileId);
