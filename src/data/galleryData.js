// Gallery images data - centralized for reuse across components
...
// Helper function to get images relevant to a travel package
export const getRelevantGalleryImages = (travelPackage) => {
  if (!travelPackage) return galleryImages.slice(0, 6);
  
  const packageLocation = travelPackage.location.toLowerCase();
  
  // First, try to find images that match the location
  let relevantImages = galleryImages.filter(img => 
    img.location.toLowerCase().includes(packageLocation.split(',')[0].toLowerCase()) ||
    packageLocation.includes(img.location.toLowerCase())
  );

  // If no location matches, categorize by destination type
  if (relevantImages.length === 0) {
    if (packageLocation.includes('paris') || packageLocation.includes('rome') || packageLocation.includes('tokyo')) {
      relevantImages = galleryImages.filter(img => img.category === 'City');
    } else if (packageLocation.includes('beach') || packageLocation.includes('island') || packageLocation.includes('coast')) {
      relevantImages = galleryImages.filter(img => img.category === 'Beach');
    } else if (packageLocation.includes('mountain') || packageLocation.includes('alpine') || packageLocation.includes('peak')) {
      relevantImages = galleryImages.filter(img => img.category === 'Mountain');
    } else {
      // Default to showing a mix of images
      relevantImages = galleryImages.slice(0, 6);
    }
  }

  // Ensure we have at least 4 images, fill with random ones if needed
  if (relevantImages.length < 4) {
    const remainingImages = galleryImages.filter(img => !relevantImages.includes(img));
    relevantImages = [...relevantImages, ...remainingImages].slice(0, 6);
  }
